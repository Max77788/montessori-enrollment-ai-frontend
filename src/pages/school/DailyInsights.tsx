...
import {
  ...
  Check, X, Phone, Mail, Users, MessageCircle
} from 'lucide-react';
      ...
      {callHistory.length > 0 && (
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            ...
            <span className="text-[10px] font-bold text-slate-400">{callHistory.length} calls</span>
          </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {call.tour_booked && (
                          <span className="flex-shrink-0 px-1.5 py-0.5 rounded text-[10px] font-bold bg-emerald-50 text-emerald-600 border border-emerald-100">Tour Booked</span>
                        )}
                        {call.enrollment_urgency && call.enrollment_urgency !== 'unknown' && (
                          <span className={`flex-shrink-0 px-1.5 py-0.5 rounded text-[10px] font-medium border ${call.enrollment_urgency === 'immediate' ? 'text-red-500 bg-red-50 border-red-100' : call.enrollment_urgency === 'within weeks' ? 'text-amber-600 bg-amber-50 border-amber-100' : 'text-slate-400 bg-slate-50 border-slate-100'}`}>
                            {call.enrollment_urgency}
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-3 mt-1 text-xs text-slate-400">
                        <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {new Date(call.received_at).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                        {call.duration_seconds > 0 && <span>· {Math.floor(call.duration_seconds / 60)}m {call.duration_seconds % 60}s</span>}
                        {call.language_spoken && call.language_spoken !== 'English' && <span>{call.language_spoken}</span>}
                      </div>
                    </div>
                    ...
                    <div className="px-6 pb-5 pt-2 bg-slate-50/50 border-t border-slate-100 animate-soft">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="space-y-2">
                          ...
                          {call.parent_phone && <div className="flex items-center gap-2 text-sm text-slate-700"><Phone className="w-3.5 h-3.5 text-slate-400" />{call.parent_phone}</div>}
                          {call.parent_email && <div className="flex items-center gap-2 text-sm text-slate-700"><Mail className="w-3.5 h-3.5 text-slate-400" />{call.parent_email}</div>}
                          <div className="flex items-center gap-2 text-sm text-slate-700"><Users className="w-3.5 h-3.5 text-slate-400" />{call.parent_name || 'Not provided'}</div>
                        </div>
                        <div className="space-y-2">
                        ...
                        <div className="space-y-2">
                          ...
                          {call.topics_of_interest?.length > 0 ? (
                            <div className="flex flex-wrap gap-1">
                              {call.topics_of_interest.map((t, i) => (
                                <span key={i} className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-blue-50 text-blue-600 border border-blue-100">{t}</span>
                              ))}
                            </div>
                            ...
                            <div className="mt-2">
                              ...
                              <ul className="text-xs text-slate-500 space-y-0.5">
                                {call.questions_asked.map((q, i) => (
                                  <li key={i} className="flex items-start gap-1"><MessageCircle className="w-3 h-3 text-slate-300 mt-0.5 flex-shrink-0" />{q}</li>
                                ))}
                              </ul>
                            </div>
                          )}
